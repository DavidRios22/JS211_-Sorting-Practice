...
const stringsToNumbs = (arr) => { 
    const pieNum = strNums.map(x => parseInt(x))
    console.log(pieNum);
}
...
const indexFinder = (arr) => {
    let atxIdx = 0
    for (let i = 0; i < arr.length; i++) {
        if (parseInt(arr[i]) + atxIdx === 512) {
            console.log(`index: ${atxIdx}, value: ${arr[i]}`)
            break
        } else {
            atxIdx += 1
        }
...
const weatherStateFinder = (arr) => {
    for (let i = 0; i < arr.length; i++) {
    weatherStates.push(Object.values(arr[i])[1])
    }
    console.log(weatherStates);
}
weatherStateFinder(weather)
...
const weatherTempFinder = (arr) => {
    for (let i = 0; i < arr.length; i++) {   
        temp = Object.values(arr[i])[6]
    } if (temp >= 15.915) {
        const find = arr.find(x => x.min_temp === temp)
        console.log(find.id);
    }
}

weatherTempFinder(weather)
